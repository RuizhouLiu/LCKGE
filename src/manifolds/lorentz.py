...
class Lorentz(LorentzOri):
    def __init__(self, k=1.0, learnable=False, max_norm=None):
    ...
    def projx(self, x: torch.Tensor, *, dim=-1) -> torch.Tensor:
        return math.project(x, k=self.k, dim=dim, max_norm=self.max_norm)
    ...
    def cinner2(self, x: torch.Tensor, y: torch.Tensor):
        x = x.clone()
        x.narrow(-1, 0, 1).mul_(-1)
        z = x.unsqueeze(-2) @ y.unsqueeze(-2).transpose(-1, -2)
        return z.squeeze()
