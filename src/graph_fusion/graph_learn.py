...
from .graph_utils import to_cuda
...
class GraphLearner(nn.Module):
    ...
    def forward(self, context, ctx_mask=None):
        ...
        if self.metric_type == 'attention':
            ...
            attention = 0
            for context_norm_i in context_norm:
                attention += torch.matmul(context_norm_i, context_norm_i.T)
            attention = attention / float(expand_weight_tensor.shape[0])
            markoff_value = 0
...
def get_binarized_kneighbors_graph(features, topk, device=None):
